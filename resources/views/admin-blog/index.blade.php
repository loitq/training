    ...
    <div class="modal fade" id="createModal" tabindex="-1" aria-labelledby="createModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{route('admin.blog.store')}}" name="form-create" method="POST" enctype="multipart/form-data">
                    @csrf
            ...
            <tr>
                @foreach ($blogs as $blog)
                    <tr>
                        ...
                        <td>{{$blog->title}}</td>
                        <td>
                            <form action="{{route('admin.blog.edit', ['id' => $blog->id])}}" name="form-edit" method="GET">
                                <button type="submit" class="btn btn-info">Edit</button>
                            </form>
                        </td>
                        <td>
                            <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#deleteModal{{$blog->id}}">Delete</button>
                            <form action="{{route('admin.blog.destroy',['id' => $blog->id])}}" name="form-edit" method="GET">
                                <!-- Modal -->
